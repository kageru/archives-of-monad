use crate::{
    data::{HasName, conditions::Condition},
...
mod tests {
    ...
    fn test_condition_template() {
        let blinded: Condition = serde_json::from_str(&read_test_file("conditions/blinded.json")).expect("Deserialization failed");
    ...
    fn test_condition_list() {
        let blinded: Condition = serde_json::from_str(&read_test_file("conditions/blinded.json")).expect("Deserialization failed");
        let deafened: Condition = serde_json::from_str(&read_test_file("conditions/deafened.json")).expect("Deserialization failed");
